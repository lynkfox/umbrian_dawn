...
appData.regions[10001e3]={name:"Yasna Zakh"},appData.factions[500029]={name:"Deathless Custodians"},appData.systems[301e5]={name:"Zarzakh",regionID:10001e3,constellationID:2001e4,factionID:500029,security:-1},appData.map.shortest[1e5]={},[1041,1269,2086,3841].forEach(function(a){appData.map.shortest[1e5][a]=0,appData.map.shortest[a][1e5]=0});
...
const automapState={pendingDecision:!1};tripwire.autoMapper=function(e,t){var i=[];if(!$("#toggle-automapper").hasClass("active")||$("#toggle-automapper").hasClass("disabled"))return!1;if(automapState.pendingDecision)return console.info("Not automapping because there is an automap dialog up"),!1;if(!tripwire.systems[e]||!tripwire.systems[t])return!1;if(e==t)return!1;if(tripwire.systems[e].regionID>12e6||tripwire.systems[t].regionID>12e6)return console.info("Not automapping into abyssal or other special space"),!1;const s=[30000142,30002187];if(s.indexOf(e)>=0||s.indexOf(t)>=0)return console.info("Not automapping into likely pod-out destination"),!1;if(tripwire.client.EVE&&tripwire.client.EVE.stationID)return!1;if(tripwire.client.EVE&&tripwire.client.EVE.shipTypeID&&$.inArray(parseInt(tripwire.client.EVE.shipTypeID),[33328,670])>=0)return!1;if(void 0!==tripwire.map.shortest[e-3e7]&&void 0!==tripwire.map.shortest[e-3e7][t-3e7])return!1;if($.map(tripwire.client.wormholes,function(i){const s=tripwire.client.signatures[i.initialID],r=tripwire.client.signatures[i.secondaryID];return s&&r&&(s.systemID==e&&r.systemID==t||s.systemID==t&&r.systemID==e)?i:null}).length>0)return!1;function r(e){return chain.data.map.rows.filter(t=>t.c[0].systemID==e).length>0}if(r(e)&&r(t))return console.info("Not automapping because both systems are already in chain"),!1;var n,a={signatures:{add:[],update:[]}};n=tripwire.systems[t].class?"Class-"+tripwire.systems[t].class:tripwire.systems[t].security>=.45?"High-Sec":tripwire.systems[t].security>0?"Low-Sec":"Null-Sec";var o=$.map(tripwire.client.wormholes,function(t){if(void 0!==tripwire.client.signatures[t.initialID]&&void 0!==tripwire.client.signatures[t.secondaryID]&&tripwire.client.signatures[t.initialID].systemID==e&&!tripwire.systems[tripwire.client.signatures[t.secondaryID].systemID]){if(appData.genericSystemTypes[tripwire.client.signatures[t.secondaryID].systemID]==n)return t;if(t.type&&appData.wormholes[t.type]&&appData.wormholes[t.type].leadsTo.replace(" ","-")==n)return t;if(!(null!==tripwire.client.signatures[t.secondaryID].systemID||t.type&&appData.wormholes[t.type]))return t}});if(o.length)if(o.length>1)$("#dialog-select-signature").dialog({autoOpen:!0,title:"Which Signature?",width:390,buttons:{Cancel:function(){$(this).dialog("close")},Ok:function(){var e=$("#dialog-select-signature [name=sig]:checked").val(),s=o[e],r=tripwire.client.signatures[s.initialID],n=tripwire.client.signatures[s.secondaryID];a.signatures.update.push({wormhole:{id:s.id},signatures:[{id:r.id},{id:n.id,systemID:t}]});tripwire.refresh("refresh",a,function(e){e.resultSet&&1==e.resultSet[0].result&&(i.push({wormhole:s,signatures:[r,n]}),$("#dialog-select-signature").dialog("close"))})}},open:function(){function i(e){const t=systemAnalysis.analyse(e);return systemRendering.renderSystem(t)}automapState.pendingDecision=!0,$("#dialog-select-signature .optionsTable tbody").empty(),document.getElementById("select-sig-from").innerHTML=i(e),document.getElementById("select-sig-to").innerHTML=i(t),$.each(o,function(e){const t=tripwire.client.signatures[o[e].initialID],i=tripwire.makeSigInfo(t,o[e]),s="<tr><td><input type='radio' name='sig' value='"+e+"' id='sig"+e+"' /></td><td class='centerAlign'>"+formatSignatureID(t.signatureID)+"</td><td class='centerAlign'>"+i.formattedType+"</td><td class='centerAlign'>"+i.leadsTo+"</td><td class='centerAlign "+o[e].life+"'>"+i.lifeText+"</td><td class='centerAlign "+o[e].mass+"'>"+o[e].mass+"</td></tr>",r=$(s);$(r).find("td").wrapInner("<label for='sig"+e+"' />"),$("#dialog-select-signature .optionsTable tbody").append(r)})},close:function(){automapState.pendingDecision=!1}});else{var l=o[0],u=tripwire.client.signatures[l.initialID],p=tripwire.client.signatures[l.secondaryID];a.signatures.update.push({wormhole:{id:l.id},signatures:[{id:u.id},{id:p.id,systemID:t}]}),i.push({wormhole:l,signatures:[u,p]})}else a.signatures.add.push({wormhole:{type:null,parent:"initial",life:"stable",mass:"stable"},signatures:[{systemID:e,type:"wormhole"},{systemID:t,type:"wormhole"}]});if(a.signatures.add.length||a.signatures.update.length){tripwire.refresh("refresh",a,function(e){e.resultSet&&1==e.resultSet[0].result&&($("#undo").removeClass("disabled"),e.results&&(viewingSystemID in tripwire.signatures.undo?tripwire.signatures.undo[viewingSystemID].push({action:"add",signatures:e.results}):tripwire.signatures.undo[viewingSystemID]=[{action:"add",signatures:e.results}]),i.length&&(viewingSystemID in tripwire.signatures.undo?tripwire.signatures.undo[viewingSystemID].push({action:"update",signatures:i}):tripwire.signatures.undo[viewingSystemID]=[{action:"update",signatures:i}]),sessionStorage.setItem("tripwire_undo",JSON.stringify(tripwire.signatures.undo)))})}};
