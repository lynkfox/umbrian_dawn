...
tripwire.autoMapper=function(e,i){var t=[];if(!$("#toggle-automapper").hasClass("active")||$("#toggle-automapper").hasClass("disabled"))return!1;if(!tripwire.systems[e]||!tripwire.systems[i])return!1;if(e==i)return!1;if(tripwire.client.EVE&&tripwire.client.EVE.stationID)return!1;if(void 0!==tripwire.map.shortest[e-3e7]&&void 0!==tripwire.map.shortest[e-3e7][i-3e7])return!1;if($.map(tripwire.client.wormholes,function(t){const s=tripwire.client.signatures[t.initialID],r=tripwire.client.signatures[t.secondaryID];return s&&r&&(s.systemID==e&&r.systemID==i||s.systemID==i&&r.systemID==e)?t:null}).length>0)return!1;var s,r={signatures:{add:[],update:[]}};s=tripwire.systems[i].class?"Class-"+tripwire.systems[i].class:tripwire.systems[i].security>=.45?"High-Sec":tripwire.systems[i].security>0?"Low-Sec":"Null-Sec";var n=$.map(tripwire.client.wormholes,function(i){if(tripwire.client.signatures[i.initialID].systemID==e&&!tripwire.systems[tripwire.client.signatures[i.secondaryID].systemID]){if(tripwire.aSigSystems[tripwire.client.signatures[i.secondaryID].systemID]==s)return i;if(i.type&&tripwire.wormholes[i.type]&&tripwire.wormholes[i.type].leadsTo.replace(" ","-")==s)return i;if(!(null!==tripwire.client.signatures[i.secondaryID].systemID||i.type&&tripwire.wormholes[i.type]))return i}});if(n.length)if(n.length>1)$("#dialog-select-signature").dialog({autoOpen:!0,title:"Which Signature?",width:390,buttons:{Cancel:function(){$(this).dialog("close")},Ok:function(){var e=$("#dialog-select-signature [name=sig]:checked").val(),s=n[e],a=tripwire.client.signatures[s.initialID],u=tripwire.client.signatures[s.secondaryID];r.signatures.update.push({wormhole:{id:s.id},signatures:[{id:a.id},{id:u.id,systemID:i}]});tripwire.refresh("refresh",r,function(e){e.resultSet&&1==e.resultSet[0].result&&(t.push({wormhole:s,signatures:[a,u]}),$("#dialog-select-signature").dialog("close"))})}},open:function(){$("#dialog-select-signature .optionsTable tbody").empty(),$.each(n,function(e){var i=tripwire.client.signatures[n[e].initialID],t="<tr><td><input type='radio' name='sig' value='"+e+"' id='sig"+e+"' /></td>"+$("#sigTable tr[data-id='"+i.id+"']").html()+"</tr>";$("#dialog-select-signature .optionsTable tbody").append(t),$("#dialog-select-signature .optionsTable tbody td").wrapInner("<label for='sig"+e+"' />")})}});else{var a=n[0],u=tripwire.client.signatures[a.initialID],l=tripwire.client.signatures[a.secondaryID];r.signatures.update.push({wormhole:{id:a.id},signatures:[{id:u.id},{id:l.id,systemID:i}]}),t.push({wormhole:a,signatures:[u,l]})}else r.signatures.add.push({wormhole:{type:null,parent:"initial",life:"stable",mass:"stable"},signatures:[{systemID:e,type:"wormhole"},{systemID:i,type:"wormhole"}]});if(r.signatures.add.length||r.signatures.update.length){tripwire.refresh("refresh",r,function(e){e.resultSet&&1==e.resultSet[0].result&&($("#undo").removeClass("disabled"),e.results&&(viewingSystemID in tripwire.signatures.undo?tripwire.signatures.undo[viewingSystemID].push({action:"add",signatures:e.results}):tripwire.signatures.undo[viewingSystemID]=[{action:"add",signatures:e.results}]),t.length&&(viewingSystemID in tripwire.signatures.undo?tripwire.signatures.undo[viewingSystemID].push({action:"update",signatures:t}):tripwire.signatures.undo[viewingSystemID]=[{action:"update",signatures:t}]),sessionStorage.setItem("tripwire_undo",JSON.stringify(tripwire.signatures.undo)))})}};
...
var chain=new function(){this.map,this.view,this.options,this.drawing,this.data={},this.newView=function(s){return this.view=new google.visualization.DataView(new google.visualization.DataTable(s)),this.view},this.activity=function(s){for(var i=0,e=(a=document.querySelectorAll("#chainMap .nodeActivity span")).length;i<e;++i)a[i].className=a[i].className+" invisible";var a;for(i=0,e=(a=document.querySelectorAll("#chainMap [data-nodeid]")).length;i<e;++i){var t=a[i].getAttribute("data-nodeid");if(s[t]){var n=s[t].shipJumps,l=s[t].podKills,r=s[t].shipKills,c=s[t].npcKills,o=a[i].getElementsByClassName("nodeActivity")[0];if(n>0){var p=o.getElementsByClassName("jumps")[0];p.className=p.className.replace(/invisible/g,""),p.setAttribute("data-tooltip",n+" Jumps")}if(l>0){var d=o.getElementsByClassName("pods")[0];d.className=d.className.replace(/invisible/g,""),d.setAttribute("data-tooltip",l+" Pod Kills")}if(r>0){var h=o.getElementsByClassName("ships")[0];h.className=h.className.replace(/invisible/g,""),h.setAttribute("data-tooltip",r+" Ship Kills")}if(c>0){var m=o.getElementsByClassName("npcs")[0];m.className=m.className.replace(/invisible/g,""),m.setAttribute("data-tooltip",c+" NPC Kills")}}}return SystemActivityToolTips.attachedElements&&SystemActivityToolTips.attachedElements.length>0&&SystemActivityToolTips.detach($("#chainMap .whEffect")),SystemActivityToolTips.attach($("#chainMap .nodeActivity > span[data-tooltip]:not(.invisible)")),s},this.occupied=function(s){for(var i in $("#chainMap [data-icon='user'], #chainMap [data-icon='user'] + .badge").addClass("invisible"),s)$("#chainMap [data-nodeid='"+s[i].systemID+"'] [data-icon='user']").removeClass("invisible"),$("#chainMap [data-nodeid='"+s[i].systemID+"'] [data-icon='user'] + .badge").removeClass("invisible").html(s[i].count);return OccupiedToolTips.attach($("#chainMap [data-icon='user']:not(.invisible)")),s},this.flares=function(s){if($("#chainMap td.node").removeClass("redNode yellowNode greenNode"),$("#chainGrid tr").removeClass("red yellow green"),s)for(var i in s.flares){var e=s.flares[i].systemID,a=s.flares[i].flare,t=($("#chainMap [data-nodeid="+e+"]").parent().addClass(a+"Node").parent().index()-1)/3*2;t>0&&$("#chainGrid tr:eq("+t+")").addClass(a).next().addClass(a)}return s},this.grid=function(){if(0==options.chain.gridlines)return $("#chainGrid tr").addClass("hidden"),!1;$("#chainGrid tr").removeClass("hidden");var s=Math.max(0,2*$(".google-visualization-orgchart-table tr:has(.node)").length-1);$("#chainGrid tr:gt("+s+")").addClass("hidden")},this.lines=function(s){function i(i,e,a,t){var n=$("#chainMap #node"+i).parent();if(0==n.length)return!1;var l=Math.ceil((n[0].cellIndex+1)/2-1),r=$(n.parent().prev().children("td.google-visualization-orgchart-lineleft, td.google-visualization-orgchart-lineright")[l]).addClass("left-"+a+" right-"+a),c=$("#chainMap #node"+e).parent();if(0==c.length||0==r.length)return!1;var o=0,p=r[0].cellIndex;n.parent().prev().find("td").each(function(s){if(o+=this.colSpan,s==p)return!1});for(var d=Math.ceil((c[0].cellIndex+1)/2-1),h=d,m=0;m<=d;m++){var y=0;n.parent().prev().prev().prev().find("td > [data-nodeid]").each(function(s){if(s==m)return y=$(this).attr("id").replace("node",""),!1}),$.map(s.map.rows,function(s){return s.c[1].v==y?s:null}).length<=0&&h--}d=h;var v=$(n.parent().prev().prev().children("td.google-visualization-orgchart-lineleft, td.google-visualization-orgchart-lineright")[d]).addClass("left-"+a+" right-"+a),u=0,f=(e=!1,!1);n.parent().prev().prev().find("td").each(function(s,i){if(u+=this.colSpan,f&&e)return!1;if(void 0!==v[0]&&v[0].cellIndex==s){if(e=!0,$(this).addClass("left-"+a),f||u==o||$(this).addClass("bottom-"+a),o==u)return!1}else if(u==o)f=!0,$(this).addClass("bottom-"+a);else if(f||e){var t=0,l=!1,r=!1;if(n.parent().prev().find("td").each(function(s){if((t+=this.colSpan)==u&&($(this).hasClass("google-visualization-orgchart-lineleft")||$(this).hasClass("google-visualization-orgchart-lineright"))){if(0==e)return l=!0,v.removeClass("left-"+a+" right-"+a),!1;1==e&&(v.removeClass("left-"+a+" right-"+a),n.parent().prev().prev().find("td").each(function(s){s>=v[0].cellIndex&&$(this).removeClass("bottom-"+a)}),r=!0)}}),l)return!1;r||$(this).addClass("bottom-"+a)}})}for(var e in s.lines)i(s.lines[e][0],s.lines[e][1],s.lines[e][2],s.lines[e][3])},this.nodes=function(s){var i={cols:[{label:"System",type:"string"},{label:"Parent",type:"string"}],rows:[]},e=["Q003","E004","L005","Z006","M001","C008","G008","A009","SML","MED","LRG","XLG"],a=[];function t(s,i){if(!s||!tripwire.systems[s])return!1;var e;tripwire.systems[s].class?e="<span class='wh'>C"+tripwire.systems[s].class+"</span>":tripwire.systems[s].security>=.45?e="<span class='hisec'>HS</span>":tripwire.systems[s].security>0?e="<span class='lowsec'>LS</span>":tripwire.systems[s].security<=0&&(e="<span class='nullsec'>NS</span>");var a=null,t=null;if(tripwire.systems[s].class){switch(tripwire.systems[s].effect){case"Black Hole":a="blackhole";break;case"Cataclysmic Variable":a="cataclysmic-variable";break;case"Magnetar":a="magnetar";break;case"Pulsar":a="pulsar";break;case"Red Giant":a="red-giant";break;case"Wolf-Rayet Star":a="wolf-rayet"}t=tripwire.systems[s].effect}var n={v:i},l="<div id='node"+i+"' data-nodeid='"+s+"'><div class='nodeIcons'><div style='float: left;'><i class='whEffect' "+(a?"data-icon='"+a+"' data-tooltip='"+t+"'":null)+"></i></div><div style='float: right;'><i data-icon='user' class='invisible'></i><span class='badge invisible'></span></div></div><h4 class='nodeClass'>"+e+"</h4><h4 class='nodeSystem'><a href='.?system="+tripwire.systems[s].name+"'>"+(options.chain.tabs[options.chain.active]&&0!=options.chain.tabs[options.chain.active].systemID?options.chain.tabs[options.chain.active].name:tripwire.systems[s].name)+"</a></h4><h4 class='nodeType'>&nbsp;</h4><div class='nodeActivity'><span class='jumps invisible'>&#9679;</span>&nbsp;<span class='pods invisible'>&#9679;</span>&nbsp;&nbsp;<span class='ships invisible'>&#9679;</span>&nbsp;<span class='npcs invisible'>&#9679;</span></div></div>";return n.f=l,n}function n(s,i,e){var a=guidance.findShortestPath(tripwire.map.shortest,[e-3e7,i.child.systemID-3e7]),t={life:"Gate",parent:{}};return t.parent.id=i.child.id,t.parent.systemID=i.child.systemID,t.parent.name=i.child.name,t.parent.type=i.child.type,t.parent.nth=i.child.nth,t.child={},t.child.id=++s,t.child.systemID=e,t.child.name=tripwire.systems[e].name,t.child.path=a,t.child.jumps=a.length-1,t.child.nth=null,{childID:s,calcNode:t}}function l(s){if(s[0]<=0)return!1;var i=parseInt(s[1]),e=o.length;for(var a in p){var t=p[a];if(-1==$.inArray(t.id,h)){var l,r,c,m,y,v,u;if("secondary"==t.parent?(l=tripwire.client.signatures[t.secondaryID],r=tripwire.client.signatures[t.initialID]):(l=tripwire.client.signatures[t.initialID],r=tripwire.client.signatures[t.secondaryID]),c=t.type,"K162",l&&l.systemID==s[0])m=l,y=r,v="K162",u=c;else{if(!r||r.systemID!=s[0])continue;m=r,y=l,u="K162",v=c}var f={};if(f.id=t.id,f.life=t.life,f.mass=t.mass,f.time=m.modifiedTime,f.parent={},f.parent.id=i,f.parent.systemID=tripwire.systems[m.systemID]?m.systemID:m.systemID+"|"+Math.floor(Math.random()*Math.floor(1e4)),f.parent.name=m.name,f.parent.type=v,f.parent.typeBM=null,f.parent.classBM=null,f.parent.nth=null,f.parent.signatureID=y.signatureID,f.child={},f.child.id=++e,f.child.systemID=tripwire.systems[y.systemID]?y.systemID:y.systemID+"|"+Math.floor(Math.random()*Math.floor(1e4)),f.child.name=y.name,f.child.type=u,f.child.typeBM=null,f.child.classBM=null,f.child.nth=null,f.child.signatureID=m.signatureID,d.push(f),o.push([f.child.systemID,f.child.id,s[2]]),h.push(f.id),$("#show-viewing").hasClass("active")&&tripwire.systems[f.child.systemID]&&!tripwire.systems[viewingSystemID].class&&!tripwire.systems[f.child.systemID].class)e=(w=n(e,f,viewingSystemID)).childID,d.push(w.calcNode),o.push([0,e]);if($("#show-favorite").hasClass("active")&&tripwire.systems[f.child.systemID])for(a in options.favorites){var w;if(!(tripwire.systems[options.favorites[a]].regionID>=11e6||tripwire.systems[f.child.systemID].regionID>=11e6))e=(w=n(e,f,options.favorites[a])).childID,d.push(w.calcNode),o.push([0,e])}}}}function r(s){if(s.length<=1||s.length>options.chain.routingLimit)return""+s.length-1;for(var i=s.slice(0,s.length-1).reverse().map(s=>{var i=appData.systems[3e7+1*s];return'<span class="'+(i.security>=.45?"hisec":i.security>=0?"lowsec":"nullsec")+'" data-tooltip="'+i.name+" ("+i.security+')">&#x25a0</span>'}),e='<span class="path">',a=0;a<i.length;a++)a>0&&0==a%5&&(e+="|"),e+=i[a];return e+"</span>"}if($("#chainTabs .current").length>0){var c=$("#chainTabs .current .name").data("tab").toString().split(","),o=[],p=s,d=[],h=[];if(0==c){let e=0;Object.keys(s).slice().forEach(a=>{const n=tripwire.client.signatures[s[a].initialID],l=tripwire.client.signatures[s[a].secondaryID];n&&tripwire.systems[n.systemID]&&void 0===tripwire.systems[n.systemID].class?(e++,i.rows.push({c:[t(n.systemID,e),{v:null}]}),o.push([n.systemID,e,n.systemID])):l&&tripwire.systems[l.systemID]&&void 0===tripwire.systems[l.systemID].class&&(e++,i.rows.push({c:[t(l.systemID,e),{v:null}]}),o.push([l.systemID,e,l.systemID]))})}else for(var m in c)i.rows.push({c:[t(c[m],parseInt(m)+1),{v:null}]}),o.push([c[m],parseInt(m)+1,c[m]]);for(var y=0;y<o.length;y++)l(o[y])}else{$("#chainError").hide();var v={c:[]},u=viewingSystemID;v.c.push(t(u,1),{v:null}),i.rows.push(v);for(o=[[u,1]],p=s,d=[],h=[],y=0;y<o.length;++y)l(o[y])}for(var m in d){var f,w=d[m],g=(v={c:[]},tripwire.systems[w.child.systemID]?tripwire.systems[w.child.systemID].class:null),b=tripwire.systems[w.child.systemID]?tripwire.systems[w.child.systemID].security:null,I="string"==typeof w.child.systemID&&w.child.systemID.indexOf("|")>=0?tripwire.aSigSystems[w.child.systemID.substring(0,w.child.systemID.indexOf("|"))]:null;f=6==g||"Class-6"==I||void 0!==tripwire.wormholes[w.child.type]&&"Class 6"==tripwire.wormholes[w.child.type].leadsTo?"<span class='wh'>C6</span>":5==g||"Class-5"==I||void 0!==tripwire.wormholes[w.child.type]&&"Class 5"==tripwire.wormholes[w.child.type].leadsTo?"<span class='wh'>C5</span>":4==g||"Class-4"==I||void 0!==tripwire.wormholes[w.child.type]&&"Class 4"==tripwire.wormholes[w.child.type].leadsTo?"<span class='wh'>C4</span>":3==g||"Class-3"==I||void 0!==tripwire.wormholes[w.child.type]&&"Class 3"==tripwire.wormholes[w.child.type].leadsTo?"<span class='wh'>C3</span>":2==g||"Class-2"==I||void 0!==tripwire.wormholes[w.child.type]&&"Class 2"==tripwire.wormholes[w.child.type].leadsTo?"<span class='wh'>C2</span>":1==g||"Class-1"==I||void 0!==tripwire.wormholes[w.child.type]&&"Class 1"==tripwire.wormholes[w.child.type].leadsTo?"<span class='wh'>C1</span>":g>6?"<span class='wh'>C"+g+"</span>":void 0!==tripwire.wormholes[w.child.type]&&tripwire.wormholes[w.child.type].leadsTo.split(" ").length>1?"<span class='wh'>C"+tripwire.wormholes[w.child.type].leadsTo.split(" ")[1]+"</span>":b>=.45||"High-Sec"==I||void 0!==tripwire.wormholes[w.child.type]&&"High-Sec"==tripwire.wormholes[w.child.type].leadsTo&&!b?"<span class='hisec'>HS</span>":b>0||"Low-Sec"==I||void 0!==tripwire.wormholes[w.child.type]&&"Low-Sec"==tripwire.wormholes[w.child.type].leadsTo&&!b?"<span class='lowsec'>LS</span>":b<=0&&null!=b||"Null-Sec"==I||void 0!==tripwire.wormholes[w.child.type]&&"Null-Sec"==tripwire.wormholes[w.child.type].leadsTo?"<span class='nullsec'>NS</span>":"<span>&nbsp;</span>";var D=null,C=null;if(void 0!==tripwire.systems[w.child.systemID]){switch(tripwire.systems[w.child.systemID].effect){case"Black Hole":D="blackhole";break;case"Cataclysmic Variable":D="cataclysmic-variable";break;case"Magnetar":D="magnetar";break;case"Pulsar":D="pulsar";break;case"Red Giant":D="red-giant";break;case"Wolf-Rayet Star":D="wolf-rayet";break;default:D=null}C=tripwire.systems[w.child.systemID].effect}var T={v:w.child.id},M="<div id='node"+w.child.id+"' data-nodeid='"+w.child.systemID+"' data-sigid='"+w.id+"'><div class='nodeIcons'><div style='float: left;'><i class='whEffect' "+(D?"data-icon='"+D+"' data-tooltip='"+C+"'":null)+"></i></div><div style='float: right;'><i data-icon='user' class='invisible'></i><span class='badge invisible'></span></div></div><h4 class='nodeClass'>"+(f+sigFormat(w.child.classBM,"class"))+"</h4><h4 class='nodeSystem'>"+(tripwire.systems[w.child.systemID]?"<a href='.?system="+tripwire.systems[w.child.systemID].name+"'>"+(w.parent.name?w.parent.name:tripwire.systems[w.child.systemID].name)+"</a>":w.parent.name?w.parent.name:"<a class='invisible'>system</a>")+"</h4>"+(w.child.path?"<h4 class='nodeType'>"+r(w.child.path)+"</h4>":"<h4 class='nodeType'>"+("id"==options.chain["node-reference"]?w.child.signatureID?w.child.signatureID.substring(0,3):"&nbsp;":(w.child.type||"&nbsp;")+sigFormat(w.child.typeBM,"type")||"&nbsp;")+"</h4>")+"<div class='nodeActivity'><span class='jumps invisible'>&#9679;</span>&nbsp;<span class='pods invisible'>&#9679;</span>&nbsp;&nbsp;<span class='ships invisible'>&#9679;</span>&nbsp;<span class='npcs invisible'>&#9679;</span></div></div>";T.f=M;var S={v:w.parent.id};v.c.push(T,S),i.rows.push(v),"critical"!=w.life||-1==$.inArray(w.parent.type,e)&&-1==$.inArray(w.child.type,e)?"critical"==w.life&&"critical"==w.mass?a.push(Array(T.v,S.v,"eol-critical",w.id)):"critical"==w.life&&"destab"==w.mass?a.push(Array(T.v,S.v,"eol-destab",w.id)):-1!=$.inArray(w.parent.type,e)||-1!=$.inArray(w.child.type,e)?a.push(Array(T.v,S.v,"frig",w.id)):"critical"==w.life?a.push(Array(T.v,S.v,"eol",w.id)):"critical"==w.mass?a.push(Array(T.v,S.v,"critical",w.id)):"destab"==w.mass?a.push(Array(T.v,S.v,"destab",w.id)):"Gate"!=w.life&&"GATE"!=w.parent.type&&"GATE"!=w.child.type||a.push(Array(T.v,S.v,"gate",w.id)):a.push(Array(T.v,S.v,"eol-frig",w.id))}return a.reverse(),{map:i,lines:a}},this.redraw=function(){var s=$.extend(!0,{},this.data);s.map=$.extend(!0,{},s.rawMap),this.draw(s)},this.draw=function(s){s=void 0!==s?s:{};if(!this.map||Object.size(s.map)&&!tripwire.client.signatures)setTimeout(()=>chain.draw(s),100);else{if(s.map){if(this.drawing=!0,this.data.rawMap=$.extend(!0,{},s.map),Object.sort(s.map,"id"),$.extend(s,this.nodes(s.map)),$.extend(this.data,s),this.map.draw(this.newView(s.map),this.options),options.chain.tabs[options.chain.active])for(var i in options.chain.tabs[options.chain.active].collapsed){var e=$("#chainMap [data-nodeid='"+options.chain.tabs[options.chain.active].collapsed[i]+"']").attr("id");e&&(e=e.split("node")[1],this.map.collapse(e-1,!0))}this.lines(s),this.grid(),$("#chainMap [data-nodeid='"+viewingSystemID+"']").parent().addClass("currentNode"),WormholeTypeToolTips.attachedElements&&WormholeTypeToolTips.attachedElements.length>0&&WormholeTypeToolTips.detach($("#chainMap .whEffect")),WormholeTypeToolTips.attach($("#chainMap .whEffect[data-icon]")),WormholeRouteToolTips.attach($(".path span[data-tooltip]")),this.drawing=!1}!this.data.activity||new Date>this.data.activity.expires?(s.activity={},tripwire.esi.universeJumps().done(function(i,e,a){s.activity.expires=new Date(a.getResponseHeader("expires")),$.each(i,function(e){s.activity[i[e].system_id]={systemID:i[e].system_id,shipJumps:i[e].ship_jumps}})}).then(function(){return tripwire.esi.universeKills().done(function(i){$.each(i,function(e){s.activity[i[e].system_id]?(s.activity[i[e].system_id].podKills=i[e].pod_kills,s.activity[i[e].system_id].shipKills=i[e].ship_kills,s.activity[i[e].system_id].npcKills=i[e].npc_kills):s.activity[i[e].system_id]={systemID:i[e].system_id,podKills:i[e].pod_kills,shipKills:i[e].ship_kills,npcKills:i[e].npc_kills}})})}).then(function(){chain.data.activity=chain.activity(s.activity)})):s.map&&chain.activity(this.data.activity),s.occupied&&(this.data.occupied=this.occupied(s.occupied)),s.flares&&(this.data.flares=this.flares(s.flares))}},this.collapse=function(s){if(chain.drawing)return!1;if(options.chain.tabs[options.chain.active]){var i=chain.map.getCollapsedNodes();for(x in options.chain.tabs[options.chain.active].collapsed=[],i){var e=$("#chainMap #node"+(i[x]+1)).data("nodeid");options.chain.tabs[options.chain.active].collapsed.push(e)}}chain.lines(chain.data),$("#chainMap [data-nodeid='"+viewingSystemID+"']").parent().addClass("currentNode"),WormholeTypeToolTips.attachedElements&&WormholeTypeToolTips.attachedElements.length>0&&WormholeTypeToolTips.detach($("#chainMap .whEffect")),WormholeTypeToolTips.attach($("#chainMap .whEffect[data-icon]")),chain.activity(chain.data.activity),chain.occupied(chain.data.occupied),chain.flares(chain.data.flares),chain.grid(),options.save()},this.init=function(){chain.map=new google.visualization.OrgChart(document.getElementById("chainMap")),chain.options={allowHtml:!0,allowCollapse:!0,size:"medium",nodeClass:"node"},google.visualization.events.addListener(chain.map,"collapse",chain.collapse),chain.map.draw(new google.visualization.DataView(new google.visualization.DataTable({cols:[{label:"System",type:"string"},{label:"Parent",type:"string"}]})),this.options)},google.charts.setOnLoadCallback(this.init)};
